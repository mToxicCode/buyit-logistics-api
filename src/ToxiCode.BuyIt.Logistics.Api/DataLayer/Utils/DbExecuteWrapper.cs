...
public class DbExecuteWrapper
{
    ...
    public async Task ExecuteQueryAsync(string query, object? parameters = null,
        CancellationToken? cancellationToken = default)
    {
        await using var db = _connectionFactory.CreateDatabase(cancellationToken);
    ...
    public async Task<T[]> MultipleExecuteQueryAsync<T>(string query, object? parameters = null,
        CancellationToken? cancellationToken = default)
    {
        await using var db = _connectionFactory.CreateDatabase(cancellationToken);
